...
__device__ void P_chasing2(int mark, int *A, long long int iterations, int *B, int *C, long long int *D, int starting_index, float clock_rate, int data_stride){//////what is the effect of warmup outside vs inside?
	...
	*/
	...
	for (long long int it = 0; it < 4096; it++){		
		C[it] = s_index[it];
		D[it] = s_tvalue[it];
	}
}
...
int main(int argc, char **argv)
{
	...
	for(int data_stride = 1 * 1 * 256; data_stride <= 1 * 1 * 256; data_stride = data_stride * 2){/////////32mb stride
	...
	for(long long int mod2 = 1073741824; mod2 <= 1073741824; mod2 = mod2 * 2){////268435456 = 1gb, 536870912 = 2gb, 1073741824 = 4gb, 2147483648 = 8gb, 4294967296 = 16gb.
		counter++;
		///////////////////////////////////////////////////////////////////CPU data begin
