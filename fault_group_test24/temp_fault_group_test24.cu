...
__global__ void stream_warp(long long int *ptr, const long long int size, long long int *output, const long long int val, long long int STRIDE_64Kx) 
{ 
...
__global__ void stream_warp1(long long int *ptr, const long long int size, long long int *output, const long long int val, long long int STRIDE_64Kx){///////////fully unrolled 
  int lane_id = threadIdx.x & 31; 
  long long int warp_id = (threadIdx.x + blockIdx.x * blockDim.x) >> 5; 
  int warps_per_grid = (blockDim.x * gridDim.x) >> 5; 
  long long int warp_total = (size + STRIDE_64K-1) / STRIDE_64K;
...
int main(int argc, char **argv)
{
	...
	*/
	...
	for(long long int STRIDE_64Kx = 256; STRIDE_64Kx <= 256; STRIDE_64Kx = STRIDE_64Kx * 2){
	//printf("############approach\n");
	...
	//}
		...
		stream_warp1<<<dimx, dimy>>>(CPU_data_in1, 8 * data_size, GPU_data_out1, 7, STRIDE_64Kx);
