...
int main(int argc, char **argv)
{
	...
	for(long long int data_stride = 1 * 16 * 1024; data_stride <= 1 * 16 * 1024; data_stride = data_stride * 2){///test
	...
	for(long long int clock_count = 1; clock_count <= 1; clock_count = clock_count * 2){/////test
		...
		Page_visitor<<<128, 512>>>(CPU_data_in, data_stride, clock_count);///////////////1024 per block max
