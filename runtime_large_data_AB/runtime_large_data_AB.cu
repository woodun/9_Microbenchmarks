...
int main(int argc, char **argv)
{
	...
	//////////see the ptx for cooperative group.
	...
	for(long long int clock_count = 128; clock_count <= 128; clock_count = clock_count * 2){
		...
		data_size = data_size * 8192 * factor;
		...
		Page_visitor<<<8192 * factor, 512>>>(CPU_data_in, GPU_data_out, data_stride, clock_count);///////////////1024 per block max
