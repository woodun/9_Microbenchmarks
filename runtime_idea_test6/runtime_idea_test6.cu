...
__global__ void page_visitorx(long long int *A1, long long int *A2, long long int *B1, double data_stride, long long int clock_count, long long int offset , long long int rate, long long int coverage){////vertical + horizontal
...
int main(int argc, char **argv)
{
	...
	*/
	...
	for(long long int clock_count = 128; clock_count <= 128; clock_count = clock_count * 2){
		...
		long long int data_size = (long long int) temp;
		//data_size = data_size * 8192 * 512 / factor;
		data_size = data_size * 512 / factor;
		...
		if(1){
			...
			*/
			...
			gpu_initialization<<<512, 512>>>(GPU_data_out1, data_stride, data_size);////////////1024 per block max
			cudaDeviceSynchronize();
			gpu_initialization<<<512, 512>>>(CPU_data_in1, data_stride, data_size);//////////1024 per block max
			cudaDeviceSynchronize();
			gpu_initialization<<<512, 512>>>(CPU_data_in2, data_stride, data_size);//////////1024 per block max
			cudaDeviceSynchronize();
		}else{
			init_cpu_data(GPU_data_out1, data_size, data_stride);
			init_cpu_data(CPU_data_in1, data_size, data_stride);
		...
		baseline<<<512, 512>>>(CPU_data_in1, CPU_data_in2, GPU_data_out1, data_stride, clock_count);
		///////////////////////////////////////////////////32 * 64 * 1 * 512 * 1024 = 8gb.
		cudaDeviceSynchronize();
