...
int main(int argc, char **argv)
{
	...
	for(long long int data_stride = 1 * 1 * 4; data_stride <= 1 * 1 * 256; data_stride = data_stride + 1 * 1 * 4){/////512 is 4m, see what happens after 2m. 128 positions.
	...
	for(long long int clock_count = 128; clock_count <= 128; clock_count = clock_count * 2){
		...
		*/
		...
		gpu_initialization<<<32768, 512>>>(CPU_data_in, data_stride, data_size);///////////////1024 per block max
		...
		Page_visitor<<<32768, 512>>>(CPU_data_in, data_stride, clock_count);///////////////1024 per block max
