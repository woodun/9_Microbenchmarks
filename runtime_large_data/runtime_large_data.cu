...
int main(int argc, char **argv)
{
	...
	for(long long int data_stride = 1 * 1 * 256; data_stride <= 1 * 1 * 256; data_stride = data_stride + 1 * 1 * 2){////test
	...
	for(long long int clock_count = 128; clock_count <= 128; clock_count = clock_count * 2){
		...
		*/
		...
		gpu_initialization<<<65536, 512>>>(CPU_data_in, data_stride, data_size);///////////////1024 per block max
		...
		Page_visitor<<<65536, 512>>>(CPU_data_in, data_stride, clock_count);///////////////1024 per block max
