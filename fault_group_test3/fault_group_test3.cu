...
__global__ void page_visitor2(long long int *A1, long long int *B1, double data_stride, long long int clock_count){////mixed 
	...
	//double temp = (blockIdx.x * blockDim.x + threadIdx.x) * 1;
	double temp = ((blockIdx.x * blockDim.x + threadIdx.x) % 32) * stride * (blockDim.x / 32) + warpid * stride;
...
int main(int argc, char **argv)
{
	...
	*/
	...
	//}
		...
		int block_num = 1;
		if(time == 0){
			page_visitor<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);/////long 
		}
		if(time == 1){
			page_visitor1<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);/////long half
		}
		if(time == 2){
			page_visitor2<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);//////mixed	
		}
		if(time == 3){
			page_visitor3<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);//////mixed	half
		}
		if(time == 4){
			page_visitor4<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);/////long pause
		}
		if(time == 5){
			page_visitor5<<<block_num, 64>>>(CPU_data_in1, GPU_data_out1, data_stride, clock_count);//////mixed	long
		}
		cudaDeviceSynchronize();
