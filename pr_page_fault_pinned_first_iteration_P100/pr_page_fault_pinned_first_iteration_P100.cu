...
int main(int argc, char **argv)
{
	...
	int counter = 0;
	for(int data_stride = 1 * 2 * 1024; data_stride <= 32 * 256 * 1024; data_stride = data_stride * 2){/////////64 * 1024 = 256kb, 128 * 1024 = 512kb, 512 * 1024 = 2mb
		//printf("###################data_stride%d#########################\n", data_stride);
	//for(int mod = 1024 * 256 * 2; mod > 0; mod = mod - 32 * 1024){/////kepler L2 1.5m
	for(long long int mod = 2 * 256 * 1024 * 8; mod <= 32 * 256 * 1024; mod = mod * 2){////268435456 = 1gb, 536870912 = 2gb, 1073741824 = 4gb, 2147483648 = 8gb, 4294967296 = 16gb.
		counter++;
		///////////////////////////////////////////////////////////////////CPU data begin
